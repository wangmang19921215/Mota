...
class object():
	def __init__(self, screen, path, x , y, o_type = basic_enum.o_type.ground, multiple = 1.5, arg = {}):
		self.screen = screen
		...
		for i in range(1,14):
			for j in range(1,14):
				if self.scene[i - 1][j - 1] == 1:
					self.objects.append(object(screen, "resources/地形/wall.png", j, i, o_type = basic_enum.o_type.wall))
				elif self.scene[i - 1][j - 1] == 4:
					self.down_floor = [j, i]
					if not data['config']['is_lowest']:
						self.objects.append(object(screen, "resources/地形/down_floor.png", j, i, o_type = basic_enum.o_type.down_floor))
				elif self.scene[i - 1][j - 1] == 5:
					self.up_floor = [j, i]
					if not data['config']['is_highest']:
						self.objects.append(object(screen, "resources/地形/up_floor.png", j, i, o_type = basic_enum.o_type.up_floor))
				elif 62 >= self.scene[i - 1][j - 1] >= 60:
					self.objects.append(door(screen, "resources/地形/門/%s 0.png" % (["黃","藍","紅"][self.scene[i - 1][j - 1] - 60]), j, i, o_type = basic_enum.o_type.door, arg = {"d_type": self.scene[i - 1][j - 1] - 60}))
...
def produce_number(screen, number,x ,y):
	...
	for i,j in enumerate(number):
		c.append(object(screen, "resources/字/%s.png" % j, x + 0.5 * i, y - 0.025, o_type = basic_enum.o_type.scene, multiple = 0.22))
	return c
...
def run_game():
	...
	for i in range(-6,0):
		scenes.append(object(screen, "resources/地形/wall 3.png", i,  0, o_type = basic_enum.o_type.scene))
		scenes.append(object(screen, "resources/地形/wall 3.png", i, 14, o_type = basic_enum.o_type.scene))
		scenes.append(object(screen, "resources/地形/wall 3.png", i, 7, o_type = basic_enum.o_type.scene))

	for i in range(15):
		scenes.append(object(screen, "resources/地形/wall 3.png", i,  0, o_type = basic_enum.o_type.wall))
		scenes.append(object(screen, "resources/地形/wall 3.png", i, 14, o_type = basic_enum.o_type.wall))
		scenes.append(object(screen, "resources/地形/wall 3.png", 0,  i, o_type = basic_enum.o_type.wall))
		scenes.append(object(screen, "resources/地形/wall 3.png", 14, i, o_type = basic_enum.o_type.wall))
		scenes.append(object(screen, "resources/地形/wall 3.png", -6, i, o_type = basic_enum.o_type.scene))

	scenes.append(object(screen, "resources/勇者/down 0.png", -4.5, 1.25, o_type = basic_enum.o_type.scene))
	scenes.append(object(screen, "resources/字/等级.png", -2.5, 0.75, o_type = basic_enum.o_type.scene,multiple = 1.2))

	scenes.append(object(screen, "resources/字/体力.png", -3.5, 2, o_type = basic_enum.o_type.scene))
	scenes.append(object(screen, "resources/字/攻击.png", -3.5, 3, o_type = basic_enum.o_type.scene))
	scenes.append(object(screen, "resources/字/防御.png", -3.5, 4, o_type = basic_enum.o_type.scene))
	scenes.append(object(screen, "resources/字/敏捷.png", -3.5, 5, o_type = basic_enum.o_type.scene))
	scenes.append(object(screen, "resources/道具/16.png", -4.5, 9, o_type = basic_enum.o_type.scene))
	scenes.append(object(screen, "resources/道具/17.png", -4.5, 10, o_type = basic_enum.o_type.scene))
	scenes.append(object(screen, "resources/道具/18.png", -4.5, 11, o_type = basic_enum.o_type.scene))
	scenes.append(object(screen, "resources/道具/31.png", -4.6, 12, o_type = basic_enum.o_type.scene))


	for i in range(-6,15):
		for j in range(0,15):
			grounds.append(object(screen, "resources/地形/ground.png", i, j, o_type = basic_enum.o_type.ground))
